...
# 1. Find empty statements in raw extraction (0 transactions)
...
# From raw extraction: files with 0 transactions
...
# 2. Also scan Empty Statements folders in data room
...
# 3. Load flat JSON for transaction counts
...
# Print NO ACTIVITY cells for reference
print('=== NO ACTIVITY CONFIRMED cells ===')
for acct in accounts:
    cells = [(y, m) for a, y, m in no_activity_cells if a == acct]
    if cells:
        cells.sort()
        print(f'  {acct}: {cells}')
...
for y, m in months:
    col_total = 0
    for acct in accounts:
        if (acct, y, m) not in no_activity_cells:
